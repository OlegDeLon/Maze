...
TEST(maze_case, open_test) {
  s21::Maze maze_;
  maze_.LoadMaze("../src/model/new_maze.txt");
...
TEST(maze_case, find_test) {
  ...
  s21::Maze maze_;
  s21::Maze_Walker maze_walker_;
  maze_.LoadMaze("../src/model/new_maze.txt");
  maze_walker_.GoToWalk("../src/model/new_maze.txt", std::pair<int, int>(0, 0),
                        std::pair<int, int>(3, 3));
  std::vector<std::pair<int, int>> way_walker_ =
      maze_walker_.ClearWay(std::pair<int, int>(0, 0), std::pair<int, int>(3, 3));

  for (int i = 0; i < way_walker_.size(); i++) {
